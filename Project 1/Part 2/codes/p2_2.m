...
%read image and contrast strech it
...
%read input input image as image
...
%contrastStretchedImage = imadjust(image,stretchlim(image),[]);

%subplot(3,2,5), imshow(contrastStretchedImage), title([imageName, ' After Contrast Stretching']); %show contrast stretched input image in subplot
%subplot(3,2,6), histogram(contrastStretchedImage(:),0:255), title([imageName, ' Histogram After Contrast Stretching']); %show contrast stretched input image histogram in subplot
